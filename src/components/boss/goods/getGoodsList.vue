<template>
    <div class="goods-list">
        <div v-if="goodsList != ''">
            ...
            >
                <van-collapse-item :title="item.goodsName" :value="taskState(item)" :name="index" >
                    <van-cell-group v-if="isedit == false">
                        <van-cell title="商品重量" :value="itemDetail.goodsWeight" />
                        <van-cell title="商品价格" :value="itemDetail.goodsPrice" />
                        <van-cell title="商品描述" :value="itemDetail.goodsDesc" />
...
export default {
    ...
    mounted() {
        axios.post('/boss/goods/list',qs.stringify(),{
    ...
    methods: {
        taskState(item) {
            if (item.goodsState == "Wait") {
                return "待领取";
            }else if (item.goodsState == "Finished") {
                return "已完结";
            }else if (item.goodsState == "Del") {
                return "已删除";
            }
        },
        itemInfo(item,index) {
            if(item.goodsId !== this.itemDetail.goodsId) {
                if(this.isedit == true) {
                    this.isedit = false
                }
                let params = {
                ...
                axios.post('/boss/goods/detail',qs.stringify(params),{
                    headers: { Token: getToken("Token") }
                }).then(res => {
                    if(res.data.code === 0) {
